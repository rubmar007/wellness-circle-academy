...
final class AdminPromocionesController
{
    private const TABLE = 'announcements';
    private const BASE  = '/admin/promociones';
    ...
    public function index(array $params): void
    {
        ...
        View::render('admin/promociones/index', [
            'promociones' => $rows,
            'flash'       => self::popFlash(),
        ]);
    }
    ...
    {
        Auth::requireAdmin();
        View::render('admin/promociones/form', [
            'mode'      => 'create',
            'promo'     => null,
            'errors'    => [],
            'old'       => self::emptyOld(),
    ...
    public function store(array $params): void
    {
        ...
        self::setFlash('Promoción creada.');
    ...
    {
        Auth::requireAdmin();
        $promo = self::findRow(self::parseId($params['id'] ?? ''));
        if (!$promo) { self::render404(); return; }
        ...
        View::render('admin/promociones/form', [
            ...
            'old'    => [
                ...
                'video_url'    => (string) ($promo['video_url'] ?? ''),
                'link_url'     => (string) ($promo['link_url'] ?? ''),
                'link_label'   => (string) ($promo['link_label'] ?? ''),
                'is_published' => $promo['is_published'] ? '1' : '',
            ],
        ]);
    }
    ...
    public function update(array $params): void
    {
        ...
        $finalImage = $newImagePath !== '' ? $newImagePath : (string) ($promo['image_path'] ?? '');

        // Si se marcó "eliminar imagen"
        if (isset($_POST['remove_image']) && $_POST['remove_image'] === '1') {
            Upload::deleteImage((string) ($promo['image_path'] ?? ''));
            $finalImage = '';
        } elseif ($newImagePath !== '') {
            Upload::deleteImage((string) ($promo['image_path'] ?? ''));
        }
        ...
        self::setFlash('Promoción actualizada.');
        View::redirect(self::BASE);
    }
    ...
    public function confirmDestroy(array $params): void
    {
        ...
        View::render('admin/promociones/delete', ['promo' => $promo]);
    }
    ...
    public function destroy(array $params): void
    {
        ...
        self::setFlash('Promoción eliminada.');
        View::redirect(self::BASE);
    }
